...
5. تأكد أن مجلد `app` ظاهر في جذر المستودع بجانب `package.json`.
...
إذا ظهر خطأ `Couldn't find any pages or app directory` فلا تعِد المحاولة قبل
رفع مجلد `app` إلى جذر فرع `main`. بعد اكتمال الرفع، استخدم **Retry build** من
صفحة تفاصيل البناء أو ادفع commit جديدًا ليبدأ البناء تلقائيًا.
